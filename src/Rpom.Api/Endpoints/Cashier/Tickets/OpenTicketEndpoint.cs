...
internal sealed class OpenTicketEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("api/cashier/tickets",
            ...
            {
                var result = await sender.Send(new OpenTicket.Command(
            ...
            .WithDescription("Request: JSON body { tableId:int, guestCount:short, shiftId:int, notes?:string }. Requires the table lock; counter/area derived from the table. Response: 201 Created — Location header; JSON body { ticketId, code }. 409 if no open cash drawer / lock not held.");
    }
