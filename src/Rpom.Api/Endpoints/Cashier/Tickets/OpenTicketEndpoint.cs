        ...
        app.MapPost("api/cashier/tickets",
                async ([FromBody] Request request, ISender sender, CancellationToken ct) =>
                {
                    Result<OpenTicket.Response> result = await sender.Send(new OpenTicket.Command(
                        request.TableId, request.GuestCount, request.Notes), ct);
            ...
            .WithDescription("""
    Request: JSON body { tableId:int, guestCount:short, notes?:string }. Requires the table lock;
    counter/area derived from the table, shift derived from the OPEN cash drawer. Response: 201 Created —
    Location header; JSON body { ticketId, code }. 409 if no open cash drawer / lock not held.
""");
    }

    internal sealed record Request(int TableId, short GuestCount, string? Notes);
}
