...
internal sealed class SetAreaMenuCategoriesEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPut("api/areas/{areaId:int}/menu-categories",
            ...
            .Produces<ApiResult<SetAreaMenuCategories.Response>>(StatusCodes.Status200OK)
            .ProducesProblem(StatusCodes.Status400BadRequest)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Replace the categories assigned to an area's menu.")
            .WithDescription("Request: route areaId (int); JSON body { categoryIds:int[] } (empty clears the menu). Response: 200 OK — JSON SetAreaMenuCategories.Response { areaId, inserted, deleted, total }.");
    }
