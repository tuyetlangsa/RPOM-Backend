...
internal sealed class ListChoiceCategoriesEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/choice-categories",
                ...
                {
                    Result<IReadOnlyList<ListChoiceCategories.Response>> result =
                        await sender.Send(new ListChoiceCategories.Query(search, isActive), ct);
                    return result.MatchOk();
                })
            .RequireAuthorization(Permissions.MasterDataView)
            .WithTags("ChoiceCategories")
            .WithName("ListChoiceCategories")
            .Produces<ApiResult<IReadOnlyList<ListChoiceCategories.Response>>>()
            .WithSummary("List choice categories with optional filters.")
            .WithDescription(
                "Request: query search?:string, isActive?:bool. Response: 200 OK — JSON array of ListChoiceCategories.Response (incl. modifierCount).");
    }
}
