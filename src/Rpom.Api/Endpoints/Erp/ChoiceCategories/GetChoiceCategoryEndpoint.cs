...
internal sealed class GetChoiceCategoryEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/choice-categories/{id:int}",
            ...
            {
                var result = await sender.Send(new GetChoiceCategory.Query(id), ct);
                return result.MatchOk();
            })
            .RequireAuthorization(Permissions.MasterDataView)
            .WithTags("ChoiceCategories")
            .WithName("GetChoiceCategory")
            .Produces<ApiResult<GetChoiceCategory.Response>>(StatusCodes.Status200OK)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Get a choice category by id (with modifiers).")
            .WithDescription("Request: route id (int). Response: 200 OK — JSON GetChoiceCategory.Response (incl. modifiers[]).");
    }
}
