        ...
        app.MapPut("api/discount-policies/{id:int}",
            async (int id, [FromBody] Request request, ISender sender, CancellationToken ct) =>
            {
                var conditions = (request.Conditions ?? Array.Empty<ConditionRequest>())
                    ...
                    .ToList();
                var result = await sender.Send(new UpdateDiscountPolicy.Command(
            ...
            .Produces<ApiResult<UpdateDiscountPolicy.Response>>(StatusCodes.Status200OK)
            .ProducesProblem(StatusCodes.Status400BadRequest)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status409Conflict)
            .WithSummary("Update a discount policy (replace-all conditions).")
            .WithDescription("Request: route id (int); JSON body { code:string, name:string, description?:string, discountType:'TICKET_THRESHOLD'|'QUANTITY_ITEM', isAutoApply:bool, daysOfWeek?:string, isActive:bool, conditions:[{ thresholdAmount?:decimal, itemId?:int, quantityThreshold?:decimal, areaId?:int, applyType:'PERCENT'|'FIXED', discountValue:decimal, displayOrder:short }] }. Response: 200 OK — JSON UpdateDiscountPolicy.Response.");
    }
