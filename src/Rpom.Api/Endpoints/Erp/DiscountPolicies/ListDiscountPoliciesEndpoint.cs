...
internal sealed class ListDiscountPoliciesEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/discount-policies",
            ...
            {
                var result = await sender.Send(new ListDiscountPolicies.Query(search, isActive, discountType), ct);
                return result.MatchOk();
            })
            .RequireAuthorization(Permissions.MasterDataView)
            .WithTags("DiscountPolicies")
            .WithName("ListDiscountPolicies")
            .Produces<ApiResult<IReadOnlyList<ListDiscountPolicies.Response>>>(StatusCodes.Status200OK)
            .WithSummary("List discount policies with optional filters.")
            .WithDescription("Request: query search?:string, isActive?:bool, discountType?:'TICKET_THRESHOLD'|'QUANTITY_ITEM'. Response: 200 OK — JSON array of ListDiscountPolicies.Response (incl. conditionCount).");
    }
}
