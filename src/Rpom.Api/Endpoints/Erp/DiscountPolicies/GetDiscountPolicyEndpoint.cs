...
internal sealed class GetDiscountPolicyEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/discount-policies/{id:int}",
            ...
            {
                var result = await sender.Send(new GetDiscountPolicy.Query(id), ct);
                return result.MatchOk();
            })
            .RequireAuthorization(Permissions.MasterDataView)
            .WithTags("DiscountPolicies")
            .WithName("GetDiscountPolicy")
            .Produces<ApiResult<GetDiscountPolicy.Response>>(StatusCodes.Status200OK)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Get a discount policy by id (with conditions).")
            .WithDescription("Request: route id (int). Response: 200 OK — JSON GetDiscountPolicy.Response (incl. conditions[]).");
    }
}
