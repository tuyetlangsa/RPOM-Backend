...
internal sealed class ListItemsEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/items",
            ...
            .WithTags("Items")
            .WithName("ListItems")
            .Produces<ApiResult<Page<ListItems.Item>>>(StatusCodes.Status200OK)
            .WithSummary("List items (paged) with optional filters.")
            .WithDescription("Request: query categoryId?:int, search?:string, isActive?:bool, pageNumber?:int=1, pageSize?:int=50. Response: 200 OK — JSON Page<ListItems.Response> (items + pagination).");
    }
}
