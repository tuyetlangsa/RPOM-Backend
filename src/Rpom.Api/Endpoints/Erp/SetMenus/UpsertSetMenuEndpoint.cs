...
internal sealed class UpsertSetMenuEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPut("api/items/{itemId:int}/set-menu",
            ...
            .Produces<ApiResult<UpsertSetMenu.Response>>(StatusCodes.Status200OK)
            .ProducesProblem(StatusCodes.Status400BadRequest)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Create or update an item's set-menu spec.")
            .WithDescription("Request: route itemId (int); JSON body { description?:string, details:[{ detailType:'COMPONENT'|'CHOICE_CATEGORY', componentItemId?:int, quantity?:decimal, isFixed?:bool, choiceCategoryId?:int, displayOrder:short }] }. Response: 200 OK — JSON UpsertSetMenu.Response { itemId, detailCount }.");
    }
    ...
    internal sealed record DetailRequest(
        string DetailType, int? ComponentItemId, decimal? Quantity, bool? IsFixed,
        int? ChoiceCategoryId, short DisplayOrder);
}
