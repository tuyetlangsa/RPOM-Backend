...
internal sealed class GetSetMenuEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("api/items/{itemId:int}/set-menu",
                ...
                {
                    Result<GetSetMenu.Response> result = await sender.Send(new GetSetMenu.Query(itemId), ct);
                    return result.MatchOk();
                })
            .RequireAuthorization(Permissions.MasterDataView)
            .WithTags("SetMenus")
            .WithName("GetSetMenu")
            .Produces<ApiResult<GetSetMenu.Response>>()
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Get the set-menu spec of an item.")
            .WithDescription(
                "Request: route itemId (int). Response: 200 OK — JSON GetSetMenu.Response (description + details). 404 if the item is not a set menu.");
    }
}
