...
internal sealed class UpdateConfigValueEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPut("api/configs/{code}",
                ...
                {
                    Result<UpdateConfigValue.Response> result =
                        await sender.Send(new UpdateConfigValue.Command(code, request.Value), ct);
                    return result.MatchOk();
                })
            .RequireAuthorization(Permissions.ConfigManage)
            .WithTags("Configurations")
            .WithName("UpdateConfigValue")
            .Produces<ApiResult<UpdateConfigValue.Response>>()
            .ProducesProblem(StatusCodes.Status400BadRequest)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithSummary("Update an existing config value by code.")
            .WithDescription(
                "Request: route code (string); JSON body { value?:string }. Response: 200 OK — JSON UpdateConfigValue.Response.");
    }
