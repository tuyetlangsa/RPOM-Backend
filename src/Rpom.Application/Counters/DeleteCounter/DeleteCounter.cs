...
public static class DeleteCounter
{
        ...
        IVersionService versionService) : ICommandHandler<Command>
    {
        public async Task<Result> Handle(Command request, CancellationToken ct)
        {
            ...
            if (entity is null) return Result.Failure(CounterErrors.NotFound);

            var hasAreas = await dbContext.Areas.AnyAsync(x => x.CounterId == request.Id, ct);
            if (hasAreas) return Result.Failure(CounterErrors.InUse);
