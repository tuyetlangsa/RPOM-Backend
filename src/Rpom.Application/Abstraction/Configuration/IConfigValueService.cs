...
public static class ConfigValueServiceExtensions
{
    ...
    {
        string? raw = await svc.GetAsync(code, ct);
    ...
    {
        string? raw = await svc.GetAsync(code, ct);
        return decimal.TryParse(raw, CultureInfo.InvariantCulture, out decimal v) ? v : fallback;
    }
    ...
    public static async Task<string> GetStringAsync(
        ...
        await svc.GetAsync(code, ct) ?? fallback;
    ...
    {
        string? raw = await svc.GetAsync(code, ct);
        return TimeOnly.TryParse(raw, CultureInfo.InvariantCulture, out TimeOnly v) ? v : fallback;
    }
}
