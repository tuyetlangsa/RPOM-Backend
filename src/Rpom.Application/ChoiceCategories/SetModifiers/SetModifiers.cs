...
public static class SetModifiers
{
    ...
    public sealed record Response(
        int ChoiceCategoryId, int Inserted, int Updated, int Deleted, int Total);
        ...
        IVersionService versionService) : ICommandHandler<Command, Response>
    {
        ...
        {
            var ccExists = await db.ChoiceCategories.AnyAsync(c => c.Id == request.ChoiceCategoryId, ct);
            if (!ccExists) return Result.Failure<Response>(ChoiceCategoryErrors.NotFound);
            ...
            if (itemIds.Count > 0)
            {
                ...
                var found = await db.Items.CountAsync(i => distinct.Contains(i.Id), ct);
                if (found != distinct.Count)
                    return Result.Failure<Response>(ChoiceCategoryErrors.ItemNotFound);
            }

            var existing = await db.Modifiers
            ...
            foreach (var input in request.Modifiers)
            {
