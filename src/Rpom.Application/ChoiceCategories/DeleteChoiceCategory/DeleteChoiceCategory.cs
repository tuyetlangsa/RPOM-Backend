...
public static class DeleteChoiceCategory
{
        ...
        IVersionService versionService) : ICommandHandler<Command>
    {
        public async Task<Result> Handle(Command request, CancellationToken ct)
        {
            ...
            if (entity is null)
            {
                return Result.Failure(ChoiceCategoryErrors.NotFound);
            }

            bool inUse = await db.SetMenuDetails.AnyAsync(d => d.ChoiceCategoryId == request.Id, ct);
            if (inUse)
            {
                return Result.Failure(ChoiceCategoryErrors.InUse);
            }
