...
namespace Rpom.Application.CancellationReasons.UpdateCancellationReason;

public static class UpdateCancellationReason
{
        ...
        IVersionService versionService) : ICommandHandler<Command, Response>
    {
        public async Task<Result<Response>> Handle(Command request, CancellationToken ct)
        {
            ...
            if (entity is null)
            {
                return Result.Failure<Response>(CancellationReasonErrors.NotFound);
            }

            string code = request.Code.Trim();
            string codeLower = code.ToLower();

            // Allow same code as current; reject if any OTHER CancellationReason uses it (case-insensitive).
            ...
            await versionService.BumpAsync(VersionScopes.Config, $"CancellationReason.Update(id={entity.Id})", ct);
        ...
        private static string BuildSummary(CancellationReason before, Command after, string normalizedCode)
        {
            ...
            return diffs.Count == 0 ? "CancellationReason updated (no changes)" : string.Join("; ", diffs);
        }
    }
}
