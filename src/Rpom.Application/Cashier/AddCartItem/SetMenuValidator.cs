...
public static class SetMenuValidator
{
    ...
    public sealed record ChoiceCategorySpec(
        int ChoiceCategoryId, short MinChoice, short? MaxChoice, IReadOnlyList<ModifierSpec> Modifiers);
    ...
    public static ValidationResult Validate(Spec spec, IReadOnlyList<Selection> selections)
    {
        ...
        foreach (var cc in spec.ChoiceCategories)
        {
            var sels = selByCc.GetValueOrDefault(cc.ChoiceCategoryId) ?? [];
            var modById = cc.Modifiers.ToDictionary(m => m.ItemId);

            // Count of distinct chosen modifiers must respect MinChoice..MaxChoice.
            var distinctCount = sels.Select(s => s.ItemId).Distinct().Count();
            if (distinctCount < cc.MinChoice) return Invalid;
            if (cc.MaxChoice is { } max && distinctCount > max) return Invalid;

            foreach (var sel in sels)
            {
                if (!modById.TryGetValue(sel.ItemId, out var mod)) return Invalid; // not an option of this CC
                if (sel.Quantity < mod.MinPerModifier || sel.Quantity > mod.MaxPerModifier) return Invalid;
