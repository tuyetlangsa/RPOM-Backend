...
public static class OpenTicket
{
        ...
        IVersionService versionService) : ICommandHandler<Command, Response>
    {
        public async Task<Result<Response>> Handle(Command request, CancellationToken ct)
        {
            ...
            var held = await guard.EnsureHeldAsync(request.TableId, staffId, ct);
            ...
            var shiftExists = await db.Shifts.AnyAsync(s => s.Id == request.ShiftId, ct);
            if (!shiftExists) return Result.Failure<Response>(TicketErrors.ShiftNotFound);
            ...
            var tableRow = await db.Tables.FirstAsync(t => t.Id == request.TableId, ct);
            tableRow.Status = TableStatus.Occupied;
            tableRow.UpdatedAt = now;

            var staff = await db.StaffAccounts.FirstAsync(s => s.Id == staffId, ct);
