...
public static class GetTicketDetails
{
    ...
    public sealed record Info(
        long TicketId, string TicketCode,
        int TableId, string TableCode, int AreaId, string AreaName,
        int CounterId, string CounterName,
        short GuestCount,
        int? WaiterStaffId, string? WaiterName,
        int? ManagerStaffId, string? ManagerName,
        string Status, DateTime OpenedAt, int DurationMinutes,
        decimal ServiceChargePercent, decimal ServiceChargeVatPercent,
        int? DiscountPolicyId, string? DiscountPolicyName,
        decimal DiscountPercent, string? DiscountReason,
        decimal Subtotal, decimal LineDiscountTotal, decimal TicketDiscountTotal,
        decimal DiscountAmount, decimal ServiceChargeAmount, decimal VatAmount,
        decimal TotalAmount, decimal RoundingAdjustment,
        decimal PaidAmount, decimal RemainingAmount, decimal RefundAmount,
        bool HasEInvoiceRequest, bool HasGuestQrToken,
        int Version, DateTime UpdatedAt);

    public sealed record ItemDetail(
        long TicketItemSumId, int ItemId, string ItemCode, string ItemName,
        string UomCode, string UomName,
        decimal UnitPrice, decimal ChoicePricePerUnit, decimal TotalQuantity,
        decimal VatPercent, decimal ServiceChargePercent, decimal ServiceChargeVatPercent,
        decimal LineDiscountPercent, decimal TicketDiscountPercent,
        decimal TotalLineSubtotal, decimal TotalDiscount, decimal TotalServiceCharge,
        decimal TotalVat, decimal TotalAmount,
        decimal DisplayPrice, decimal DisplayLineTotal);

    public sealed record OrderBatch(
        long OrderId, short OrderNumber, string Status, DateTime? SentAt,
        int? CreatedByStaffId, string? CreatedByStaffName, string? Notes,
        IReadOnlyList<OrderedItem> Items);

    public sealed record OrderedItem(
        long OrderItemId, int ItemId, string ItemCode, string ItemName,
        string UomCode, string UomName, decimal Quantity,
        decimal UnitPrice, decimal ChoicePricePerUnit,
        decimal LineSubtotal, decimal LineTotal,
        string Status, int? KitchenStationId, string? KitchenStationName,
        DateTime SentAt, DateTime? StartCookAt, DateTime? ReadyAt, DateTime? DoneAt,
        int? CancellationReasonId, string? CancellationReasonName, string? CancellationNote,
        string? Notes,
        IReadOnlyList<ItemComponent> Components);

    public sealed record OrderingItem(
        long CartItemId, long OrderId, int ItemId, string ItemCode, string ItemName,
        string UomCode, string UomName, decimal Quantity,
        decimal UnitPrice, decimal ChoicePricePerUnit,
        decimal LineSubtotal, decimal ServiceChargeAmount, decimal VatItemAmount,
        decimal VatScAmount, decimal VatAmount, decimal LineTotal,
        string? Notes, DateTime CreatedAt, DateTime UpdatedAt,
        IReadOnlyList<ItemComponent> Components);

    public sealed record ItemComponent(
        int ItemId, string ItemName, string ComponentType,
        decimal Quantity, decimal ExtraPrice, string? Notes);

    public sealed record PaymentSection(
        decimal TotalAmount, decimal PaidAmount, decimal RemainingAmount, decimal RefundAmount,
        bool IsFullyPaid, bool HasPendingPayment,
        IReadOnlyList<PaymentLine> Payments);

    public sealed record PaymentLine(
        long TicketPaymentDetailId, int PaymentMethodId, string PaymentMethodCode,
        string PaymentMethodName, decimal Amount, string Status,
        DateTime? ProcessedAt, int ProcessedByStaffId, string ProcessedByStaffName,
        string? TransactionRef, string? Notes, DateTime CreatedAt, DateTime UpdatedAt);
        ...
        : IQueryHandler<Query, Response>
    {
        public async Task<Result<Response>> Handle(Query request, CancellationToken ct)
        {
            ...
            if (t is null) return Result.Failure<Response>(TicketErrors.NotFound);
            ...
            var orderItemDetails = await db.OrderItemDetails
                ...
                .Select(d => new { d.OrderItemId, d.ItemId, d.ItemName, d.ComponentType, d.Quantity, d.ExtraPrice, d.Notes })
                .ToListAsync(ct);
            ...
            var orderBatches = orderRows.Select(o => new OrderBatch(
                ...
                orderedItemsByOrder.GetValueOrDefault(o.Id) ?? new())).ToList();

            // --- Ordering (Order Status == DRAFT) ---
            var draftOrderIds = await db.Orders
                .Where(o => o.TicketId == ticketId && o.Status == OrderStatus.Draft)
            ...
            var cartItemDetails = await db.CartItemDetails
                ...
                .Select(d => new { d.CartItemId, d.ItemId, d.ItemName, d.ComponentType, d.Quantity, d.ExtraPrice, d.Notes })
                .ToListAsync(ct);
