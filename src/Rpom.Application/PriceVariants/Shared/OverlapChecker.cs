...
public static class OverlapChecker
{
    ...
    {
        var draftSpec = Specificity(draft);
        foreach (var s in siblings)
        {
            if (s.Id == draft.Id) continue;
            if (Specificity(s) != draftSpec) continue;
    ...
    private static bool TimeOverlap(VariantSnapshot a, VariantSnapshot b)
    {
        ...
        var aAllDay = a.BeginTime is null && a.EndTime is null;
        var bAllDay = b.BeginTime is null && b.EndTime is null;
    ...
    {
        if (a.DayMask is null || b.DayMask is null) return true;
        return (a.DayMask.Value & b.DayMask.Value) != 0;
    }

    private static bool AreaOverlap(VariantSnapshot a, VariantSnapshot b)
    {
        if (a.AppliesToAllAreas || b.AppliesToAllAreas) return true;
        // Cả 2 đều chỉ định area cụ thể → cần giao
        if (a.AreaIds.Count == 0 || b.AreaIds.Count == 0) return false;
        var set = new HashSet<int>(a.AreaIds);
        foreach (var id in b.AreaIds)
