...
public static class CreatePriceVariant
{
        ...
        IVersionService versionService) : ICommandHandler<Command, Response>
    {
        ...
        {
            // 1. Parent exists
            var parentExists = await dbContext.PriceTables.AnyAsync(t => t.Id == request.PriceTableId, ct);
            if (!parentExists) return Result.Failure<Response>(PriceVariantErrors.PriceTableNotFound);
            ...
            var siblings = await LoadSiblings(request.PriceTableId, excludeId: 0, ct);
            ...
            dbContext.AuditLogs.Add(new AuditLog
            {
                ...
                Timestamp = now,
                Summary = $"PriceVariant created: {entity.Code} — {entity.Name} (table={entity.PriceTableId}, spec={OverlapChecker.Specificity(draft)})",
            });
            await dbContext.SaveChangesAsync(ct);
