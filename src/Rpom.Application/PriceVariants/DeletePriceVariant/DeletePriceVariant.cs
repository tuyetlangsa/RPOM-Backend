...
public static class DeletePriceVariant
{
        ...
        IVersionService versionService) : ICommandHandler<Command>
    {
        public async Task<Result> Handle(Command request, CancellationToken ct)
        {
            ...
            if (entity is null)
            {
                return Result.Failure(PriceVariantErrors.NotFound);
            }

            bool hasEntries = await dbContext.PriceEntries.AnyAsync(e => e.PriceVariantId == request.Id, ct);
            if (hasEntries)
            {
                return Result.Failure(PriceVariantErrors.HasEntries);
            }

            List<PriceVariantArea> areas = await dbContext.PriceVariantAreas
                .Where(a => a.PriceVariantId == request.Id)
                .ToListAsync(ct);
            if (areas.Count > 0)
            {
                dbContext.PriceVariantAreas.RemoveRange(areas);
            }
